...
public sealed class FileTypeValidator : IFileTypeValidator
{
    ...
    {
        return IsJpg(bytes) || IsPng(bytes) || IsJpeg(bytes) || IsGif(bytes) || IsBmp(bytes) || IsTiff(bytes);
    }
    private static bool IsJpg(byte[] b)=>
        b.Length > 2 && b[0] == 0xFF && b[1] == 0xD8 && b[b.Length - 2] == 0xFF && b[b.Length - 1] == 0xD9;
